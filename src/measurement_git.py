...
def main():
    ...
    with open(os.path.realpath(os.path.join(ROOT_DIR, '../dw_circle.txt'))) as f:
        ...
        while (line != ''):
            ...
            print("Diametro effettivo : " +
                  str(float(line[6])/pixel_per_metric) + " cm")
    ...
    cv.imwrite(os.path.realpath(os.path.join(
        ROOT_DIR, '../resHough.jpg')), hough_image)
    ...
    with open(os.path.realpath(os.path.join(ROOT_DIR, '../dw_ellipse.txt'))) as f:
        ...
        while (line != ''):
            ...
            estimated_axes = estimator.DetectorEllipses(
                cropped, ellipse_image[tl[1]:bl[1], tl[0]:br[0], :], (ref)/4.0)

            print("DimA calcolata: " + str(estimated_axes[0])+" cm")
            print("DimA effettiva : " +
                  str(float(line[6])/pixel_per_metric) + " cm")
            print("DiB calcolata: " + str(estimated_axes[1])+" cm")
            print("DiamB effettiva : " +
                  str(float(line[7])/pixel_per_metric) + " cm")
    ...
    print("\nErrore medio dimA: " + str(accumulator_dimA/counter) + " cm")
    print("Errore medio dimB: " + str(accumulator_dimB/counter) + " cm")

    cv.imwrite(os.path.realpath(os.path.join(
        ROOT_DIR, '../resEllipse.jpg')), ellipse_image)
